...
env-up:	
	...
	printf "[defaults]\ninventory = inventory\n" > ansible.cfg
	printf "private_key_file = /home/ec2-user/user-to-ansible-private-key.pem" >> ansible.cfg
	printf "[nodes]\n" > inventory
	terraform output -raw wordpress_public_ip >> inventory
	printf "#!/bin/sh\n\n" > scp-ansible-config.sh
	printf "scp -q -o StrictHostKeyChecking=no -i user-to-ansible-private-key.pem ansible-to-wordpress-private-key.pem ansible.cfg inventory ./playbooks/* ec2-user@" >> scp-ansible-config.sh
